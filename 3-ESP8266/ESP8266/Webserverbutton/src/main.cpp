...
void handleRoot()
{
  esp8266_server.send(200, "text/html", "<form action=\"/LED\" method=\"POST\"><input type=\"submit\" value=\"Toggle LED\"></form></form>");
  esp8266_server.send(200, "text/html", "<form action=\"/Button\" method=\"POST\"><input type=\"submit\" value=\"check Buuton\"></form>");
}
...
void setup(void)
{
  ...
  wifiMulti.addAP("72B319", "xiao760914aa"); // 将需要连接的一系列WiFi ID和密码输入这里
  ...
  esp8266_server.on("/Button", handleButton);      // 设置服务器根目录即'/'的函数'handleRoot'
